...
  const [activeMenu, setActiveMenu] = useState("GPS");
  const [isGyroRunning, setIsGyroRunning] = useState(false); 
...
  };
  ...
  // --- Fungsi callback dibungkus dengan useCallback untuk efisiensi ---
  const handleGyroStatusChange = useCallback((runningStatus) => {
    setIsGyroRunning(runningStatus);
  }, []);

  // --- State untuk Halaman GPS (Tidak diubah) ---
  const defaultInput = {
    latitude: 58.430493,
    longitude: 15.732333,
    variation: 6.4,
...
    configSaved,
  } = useSensorConfig("gps", defaultConfig);
...
    latitude: null, longitude: null, sog: null, cog: null, variation: null,
  });

  const handleSaveAndCloseControl = (newConfig, shouldClose) => {
...
        wsClient.current.onopen = () => console.log(" WebSocket Connected");
...
                if (isSimulationActive) setDisplayData(newData);
                return;
              }
              if (isSimulationActive) {
                setDisplayData(newData);
...
                  if (!is_running) setIsEditingData(false);
                }
              }
            }
          } catch (error) {
...
        };
        wsClient.current.onerror = (error) => console.error("WebSocket Error:", error);
      } catch (error) {
...
        } else {
          console.error("Axios Error:", error.response || error);
          if (isMounted) setStatus("API Error");
        }
      }
    };
    fetchInitialData();
...
        <div className={`sidebar-icon${activeMenu === "Gyro" ? " active" : ""}`} onClick={() => setActiveMenu("Gyro")}> <img src={isGyroRunning ? iconGyroActive : iconGyroDefault} alt="Gyro" /> <span className="sidebar-label">Gyroscope</span> </div>
...
        <ConfirmationPopup isOpen={confirmPopup.isOpen} onClose={closeConfirmPopup} onConfirm={confirmPopup.onConfirm} title={confirmPopup.title} message={confirmPopup.message} />
...
              config={config}
              onSave={handleSaveAndCloseControl} 
...
                  <div className="status-section"><div className="status-container"><div className="status-item"><span className="status-label">STATUS</span><div className="status-value-container">{renderStatusImage()}</div></div><div className="status-item"><span className="status-label">UPDATE RATE</span><div className="update-rate-value">{config.updateRate} ms</div></div></div></div>
                </div>
                <div className="position-section"><h3>Position</h3><div className="coordinate-group"><div className="coordinate-box"><div className="coordinate-value">{toDMS(displayData.latitude, true)}</div><span className="coordinate-label">LATITUDE</span></div><div className="coordinate-box"><div className="coordinate-value">{toDMS(displayData.longitude, false)}</div><span className="coordinate-label">LONGITUDE</span></div></div></div>
...
                </div>
                <div className="action-buttons-container"><div className="action-buttons-group"><button className="action-icon-btn" onClick={handleStartSimulation} disabled={isDisabled || isSimulationActive}><img src={iconSend} alt="Start" /></button><button className="action-icon-btn" onClick={handleStopSimulation} disabled={isDisabled || !isSimulationActive}><img src={iconStop} alt="Stop" /></button></div><div className="action-divider"></div><button className="action-delete-btn" onClick={() => openConfirmPopup("clearData")} disabled={isDisabled || isSimulationActive}><img src={iconDelete} alt="Delete" /><span>Delete</span></button></div>
              </div>
              <div className={`edit-data-panel ${isDisabled ? "disabled" : ""}`}>
                <div className="panel-header"><h2>Edit Data</h2><div className="panel-header-actions"><button className="panel-icon-btn btn-edit-icon" onClick={handleToggleAndApplyEdit} disabled={isDisabled || isSimulationActive}><img src={iconEditPanel} alt="Edit" /></button><button className="panel-icon-btn btn-delete-icon" onClick={() => openConfirmPopup("clearInput")} disabled={isDisabled || isSimulationActive}><img src={iconDeletePanel} alt="Delete" /></button></div></div>
                {editWarning && <div className="edit-panel-warning">{editWarning}</div>}
                <div className="input-group"><label>Latitude</label><input type="number" ref={(el) => (inputRefs.current.latitude = el)} defaultValue={editData.latitude} onBlur={handleInputBlur("latitude")} step="any" disabled={isDisabled || !isEditingData || isSimulationActive}/></div>
                <div className="input-group"><label>Longitude</label><input type="number" ref={(el) => (inputRefs.current.longitude = el)} defaultValue={editData.longitude} onBlur={handleInputBlur("longitude")} step="any" disabled={isDisabled || !isEditingData || isSimulationActive}/></div>
                <div className="input-group"><label>Speed Over Ground</label><input type="number" ref={(el) => (inputRefs.current.sog = el)} defaultValue={editData.sog} onBlur={handleInputBlur("sog")} step="any" disabled={isDisabled || !isEditingData || isSimulationActive}/></div>
                <div className="input-group"><label>Course Over Ground</label><input type="number" ref={(el) => (inputRefs.current.cog = el)} defaultValue={editData.cog} onBlur={handleInputBlur("cog")} step="any" disabled={isDisabled || !isEditingData || isSimulationActive}/></div>
                <div className="input-group"><label>Magnetic Variation</label><input type="number" ref={(el) => (inputRefs.current.variation = el)} defaultValue={editData.variation} onBlur={handleInputBlur("variation")} step="any" disabled={isDisabled || !isEditingData || isSimulationActive}/></div>
                <div className="edit-panel-actions"><button className="btn-default" onClick={handleResetForm} disabled={isDisabled || isSimulationActive}>Default</button><button className="btn-save" onClick={handleSendData} disabled={isDisabled || isSimulationActive}>Save</button></div>
              </div>
            </div>
          </GpsPage>
        )}
...
        {activeMenu === "Gyro" && <GyroPage onStatusChange={handleGyroStatusChange} />}
