...
const AppContent = () => {
  ...
  const calcMagneticVariation = (lat, lon) => { 
    ...
    return parseFloat((2.5 * Math.sin((lat / 90) * Math.PI) + 1.2 * Math.cos((lon / 180) * Math.PI)).toFixed(2)); 
  };
  ...
  const handleStopSimulation = async () => { 
    ...
    try { 
      const response = await axios.patch('http://localhost:8080/api/gps', { is_running: false });
  ...
  const handleToggleAndApplyEdit = () => {
    if (isEditingData) {
      ...
      const newVariation = calcMagneticVariation(Number(currentValues.latitude), Number(currentValues.longitude));
              ...
              <div className={`edit-data-panel ${isDisabled ? 'disabled' : ''}`}>
                ...
                <div className="input-group">
                  ...
                  <input 
                    ...
                    step="any"
                    disabled={isDisabled || !isEditingData || isSimulationActive}
                  />
                </div>
                <div className="input-group">
